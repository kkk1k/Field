...
export default function ContactForm() {
  ...
  function enteredHandler(event) {
    ...
    setEmailForm(""); // emailForm 상태 초기화
    enteredTitle.current.value = "";
    enteredText.current.value = "";
  }

  return (
    <form onSubmit={enteredHandler}>
      <section>
        <h4>문의 유형</h4>
      ...
      <div>
        ...
        {emailForm === "" ? (
          <input type='text' value={emailBack} onChange={emailBackHandler} ref={enteredBackEmail} />
        ) : (
          <input type='text' value={emailForm} disabled ref={enteredBackEmail} />
        )}
        <select id='emailAdrress' name='emailAdrress' onChange={emailFormHandler}>
